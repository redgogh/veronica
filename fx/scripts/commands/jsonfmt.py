...
def reg(subparsers):
    ...
    parser.add_argument('input', type=str, nargs='?', help='JSON字符串')
...
def handle(args):
    ...
    if jsonval is None:
        jsonval = pyperclip.paste()

    console.write('-- Source Format --', color=console.RED)
    console.write(f'{jsonval}')

    console.write('')

    console.write('-- Destination Format --', color=console.RED)
    console.write(json.dumps(json.loads(jsonval), indent=2, ensure_ascii=False), color=console.GREEN)
